...
public class EmailUtility {
	...
	public static boolean sendEmail(String fromAddress, final String to, final String subject, String message, String fromName, String toName) {
		try {
			final Session session = getRPMailSession();//getMailSession(to);
			...
			mailMessage.setRecipients(RecipientType.TO, StringUtilities.isEmpty(toName) ? InternetAddress.parse(to, false) 
																						: new InternetAddress[]{new InternetAddress(to, toName)});
			mailMessage.setSubject(subject);
			mailMessage.setContent(message, "text/html;charset=UTF-8");
			
			
			boolean secure = Configuration.getEmailSecure();
			if (secure) {
				new Thread() {
					@Override
					public void run() {
						String user = Configuration.getAdminEmailAddress();
						String pass = Configuration.getEmailPass();
						
						Transport transport;
						try {
							transport = session.getTransport("smtp");
							transport.connect(/*host*/null, user, pass);
					        mailMessage.saveChanges();
					        transport.sendMessage(mailMessage, mailMessage.getAllRecipients());
						}
						catch (Exception e) {
							LOG.error("Failed to send mail; recipient=" + to + ", subject=" + subject, e);
						}
	...
	private static Session getRPMailSession() {
		Properties properties = System.getProperties();

		// Setup mail server
		properties.setProperty("mail.smtp.host", Configuration.getEmailHost());
		properties.setProperty("mail.smtp.user", Configuration.getAdminEmailAddress());
